...
import { fixVideoDuration } from './fixDuration';
...
const params = { id: 'video-1', duration: 120, taskId: 'task-1' };

describe('fixVideoDuration', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('updates duration and completes the task in one mutation', async () => {
    vi.mocked(hasuraClient.request).mockResolvedValueOnce({
      update_videos_by_pk: { id: 'video-1' },
      update_tasks: { affected_rows: 1 },
    });

    await fixVideoDuration(params);

    expect(hasuraClient.request).toHaveBeenCalledWith({
      document: expect.stringContaining('mutation FixVideoDuration'),
      variables: params,
    });
  });
  ...
  it('throws when the video was not found', async () => {
    ...
    await expect(fixVideoDuration(params)).rejects.toThrow(
